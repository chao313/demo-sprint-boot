package demo.spring.boot.demospringboot.swagger;
...
public class Swagger2Configuration {
    ...
    @Bean
    public Docket RestJpaApi() {
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("REST-API")
                .select().apis(RequestHandlerSelectors.basePackage("demo.spring.boot.demospringboot.controller.rest"))
    ...
    public Docket JpaApi() {
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("Jpa-API")
                .select().apis(RequestHandlerSelectors.basePackage("demo.spring.boot.demospringboot.controller.jpa"))
                .paths(PathSelectors.any())
                .build()
                .genericModelSubstitutes(DeferredResult.class)//异步http请求
                .forCodeGeneration(true)
